...
class Identicon(object):
    ...
    def generate(self):
        ...
        dir_path = os.path.join(current_app.config['AVATAR_PATH'], date)
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)
        filename = os.path.join(dir_path, self.id+'.png')
        filename_for_return = os.path.join(os.path.join('avatars', date), self.id+'.png')
        with open(filename, 'wb') as out:
            self.image.save(out, 'PNG')
        return filename_for_return
        #self.image.show()
